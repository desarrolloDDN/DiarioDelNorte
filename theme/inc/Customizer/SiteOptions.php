...
final class SiteOptions {
	...
	public function customize( WP_Customize_Manager $wp_customize ): void {
		...
		$this->text( $wp_customize, 'ddn_contact_form_email', __( 'Correo que recibe el formulario de contacto', 'diario-del-norte' ), '', 'sanitize_email', 'ddn_contact', __( 'En blanco: llega al correo de redacción de arriba.', 'diario-del-norte' ) );
		...
		$defaults = Social::defaults();
		foreach ( Social::catalog() as $key => $data ) {
			$this->text(
				$wp_customize,
				"ddn_social_{$key}",
				/* translators: %s: nombre de la red social. */
				sprintf( __( '%s (URL)', 'diario-del-norte' ), $data['label'] ),
				(string) ( $defaults[ $key ] ?? '' ),
				'esc_url_raw',
				'ddn_footer',
